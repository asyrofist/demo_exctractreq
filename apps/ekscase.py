...
def app():
    ...
    if 'ucdreq' in metode:
        ...
        if dataset1 and dataset2 is not None:
            ...
            tbl_5.columns = ucd1.dropna().usecase
            tbl_6 = useCaseMeasurement(freqs.aksi, data_ucd)
            tbl_6.index = freqs.id
            tbl_6.columns = useCaseTable.name
        ...
        elif 'wsd1' in tabulasi_ucdreq and dataset1 and dataset2 is not None:
            st.table(tbl_4)
        elif 'wsd2' in tabulasi_ucdreq and dataset1 and dataset2 is not None:
            st.table(tbl_5)
        elif 'wsd3' in tabulasi_ucdreq and dataset1 and dataset2 is not None:
            st.table(tbl_6)
        else:
